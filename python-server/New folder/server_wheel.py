...
# Función para liberar el dispositivo si está ocupado
def ensure_device_free(device_id):
    status = vjoy.GetVJDStatus(device_id)
    if status in [VJD_STAT_BUSY, VJD_STAT_MISS]:
        logging.info(f"Trying to relinquish device {device_id}...")
...
def acquire_vjd(device_id):
    ensure_device_free(device_id)  # Asegurarse de que el dispositivo esté libre antes de adquirirlo
    status = vjoy.GetVJDStatus(device_id)
...
# Adquirir los dispositivos vJoy
device_ids = [1, 2]
...
last_steering_value = {1: 16384, 2: 16384}  # Valores iniciales centrados para ambos dispositivos
...
def process_critical_message(message):
    ...
    try:
        if command in ['D', 'E', 'F', 'G', 'VOLUME_UP', 'VOLUME_DOWN'] and value is None:
            ...
            device_id = 1 if command in ['D', 'E', 'VOLUME_UP'] else 2
            set_button(device_id, button_id, True)
            logging.info(f"Button {button_id} on device {device_id} pressed ({command_map[command]})")
            time.sleep(0.05)
...
def process_non_critical_message(message):
    ...
    try:
        if command == 'A' and value is not None:
            ...
            device_id = 1 if y < 0 else 2  # Decidir el dispositivo basado en el valor
            steering_value = low_pass_filter(steering_value, last_steering_value[device_id])
            last_steering_value[device_id] = steering_value
            set_axis(device_id, 0x30, int(steering_value))
            logging.info(f"Steering value mapped for device {device_id}: {int(steering_value)}")
        elif command in ['B', 'C'] and value is not None:
            int_value = int(value)
            axis_value = map_value(int_value, 0, 100, 1, 32767)
            axis_id = 0x32 if command == 'B' else 0x35
            device_id = 1 if command == 'B' else 2
            set_axis(device_id, axis_id, axis_value)
            logging.info(f"{command_map[command]} value mapped for device {device_id}: {axis_value}")
        else:
            logging.error(f"Error: Unknown or improperly formatted command: {command}")
    except ValueError as ve:
        logging.error(f"Error converting value: {ve}")
